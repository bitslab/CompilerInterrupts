...
perf_test() {
  ...
  do
    ...
    *)
    ...
    declare orig_time_thr1 periodic_time_thr1 naive_time_thr1
    for thread in $THREADS
    do
      PER_THREAD_STAT_FILE="$DIR/parsec-perf_stats-th$thread-ad$AD.txt"
    ...
    BUILD_LOG=$BUILD_DEBUG_FILE ERROR_LOG=$BUILD_ERROR_FILE ALLOWED_DEVIATION=$AD CLOCK_TYPE=1 PUSH_INTV=$PI CMMT_INTV=$CI INST_LEVEL=3 PROFILE_FLAGS="-DAVG_STATS" make -f Makefile.ci ${bench}
    for thread in $THREADS
    do
      PER_THREAD_STAT_FILE="$DIR/parsec-perf_stats-th$thread-ad$AD.txt"
    ...
    do
      ...
      echo -e "\t$lc_periodic_time" >> $PER_THREAD_STAT_FILE
