...
get_lc() {
  ...
  case "$1" in
    ...
    ocean-cp) 
      ...
      ./OCEAN -n1026 -p $THREADS -e1e-07 -r2000 -t28800 > ../../out
      cd - > /dev/null
    ;;
    ocean-ncp) 
      cd ocean/non_contiguous_partitions > /dev/null
      ./OCEAN -n258 -p $THREADS -e1e-07 -r2000 -t28800 > ../../out
      cd - > /dev/null
    ;;
    barnes)
      cd barnes > /dev/null
    ...
    fmm)
      cd fmm > /dev/null
      ./FMM < inputs/input.65535.$THREADS > ../out
      cd - > /dev/null
    ;;
    raytrace)
      cd raytrace > /dev/null
      ./RAYTRACE -p $THREADS -m72 inputs/balls4.env > ../out
      cd - > /dev/null
    ;;
    radiosity)
      cd radiosity > /dev/null
      ./RADIOSITY -p $THREADS -batch -largeroom > ../out
      cd - > /dev/null
    ;;
  esac
}
