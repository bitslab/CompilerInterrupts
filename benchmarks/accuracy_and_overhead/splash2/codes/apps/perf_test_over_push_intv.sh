#!/bin/bash
CI=1000
PI="2000 5000 10000 15000 20000"
RUNS=5
AD="100"
DIR=splash2_stats
...
perf_test() {
  ...
  for bench in $*
  do
    PER_BENCH_ORIG_STAT_FILE="$DIR/$bench-push_intv_vs_perf_orig-cl$CLOCK.txt"
    PER_BENCH_LC_STAT_FILE="$DIR/$bench-push_intv_vs_perf_lc-cl$CLOCK.txt"
    PER_BENCH_ORIG_FIBER_STAT_FILE="$DIR/$bench-push_intv_vs_perf_orig_fiber-cl$CLOCK.txt"
    PER_BENCH_LC_FIBER_STAT_FILE="$DIR/$bench-push_intv_vs_perf_lc_fiber-cl$CLOCK.txt"
    ...
    echo "Push Interval, Interval Error(in Cycles), Interval Deviation of Error(in Cycles),Interval Deviation(Instruction Count),Interval Deviation of Error(Instruction Count),Duration(in ms)" | tee -a $DEBUG_FILE $PER_BENCH_LC_STAT_FILE $PER_BENCH_LC_FIBER_STAT_FILE > /dev/null
    for pi in $PI
    do
      #3. Build original program with pthread & LC
      echo "Building pthread program with logical clock (Push Intv : $pi): " | tee -a $DEBUG_FILE $BUILD_DEBUG_FILE $BUILD_ERROR_FILE >/dev/null
      ...
      echo -n "$pi," >> $PER_BENCH_LC_STAT_FILE
      echo "Push Interval: $pi" >> $DEBUG_FILE
      get_time $bench 1 $PER_BENCH_LC_STAT_FILE $pi
    done
    ...
    if [ $fiber_working -eq 1 ]; then
      ...
      do
        ...
        echo "Building pthread program with logical clock & fiber (Push Intv : $pi): " | tee -a $DEBUG_FILE $BUILD_DEBUG_FILE $BUILD_ERROR_FILE >/dev/null
        ...
        echo -n "$pi," >> $PER_BENCH_LC_FIBER_STAT_FILE
        echo "Push Interval: $pi" >> $DEBUG_FILE
        get_time $bench 1 $PER_BENCH_LC_FIBER_STAT_FILE $pi
      done
    fi
  done
}
