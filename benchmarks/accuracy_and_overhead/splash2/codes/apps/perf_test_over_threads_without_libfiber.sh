...
perf_test() {
  ...
  do
    ...
    if [ $PASSED_LC -eq 1 ]; then
      echo -e "benchmark\torig_clock\tnaive_ci\topt_ci_periodic\topt_ci_eager\tpassed_naive_ci\tpassed_periodic_ci\tpassed_eager_ci" > $PER_THREAD_STAT_FILE
    else
  ...
  do
    ...
    echo "PThread - $RUNS runs" > $BENCH_LOG
    for thread in $THREADS
    do
      PER_THREAD_STAT_FILE="$DIR/perf_stats-th$thread-ad$AD.txt"
    ...
    echo "Naive TL - $RUNS runs" > $BENCH_LOG
    for thread in $THREADS
    do
      PER_THREAD_STAT_FILE="$DIR/perf_stats-th$thread-ad$AD.txt"
    ...
    echo "Periodic TL - $RUNS runs" > $BENCH_LOG
    for thread in $THREADS
    do
      PER_THREAD_STAT_FILE="$DIR/perf_stats-th$thread-ad$AD.txt"
    ...
    echo "Eager TL - $RUNS runs" > $BENCH_LOG
    for thread in $THREADS
    do
      PER_THREAD_STAT_FILE="$DIR/perf_stats-th$thread-ad$AD.txt"
    ...
    if [ $PASSED_LC -eq 1 ]; then
      echo "Inside $PASSED_LC. $bench"
      if [ $bench == "radiosity" ]; then
        echo "Not taking passed-config benchmarks for radiosity";
      else
        #run naive passed LC
        echo "Building naive program: " >> $DEBUG_FILE
        BUILD_LOG=$BUILD_DEBUG_FILE ERROR_LOG=$BUILD_ERROR_FILE make -f Makefile.lc $bench-clean
        ...
        echo "Naive Passed - $RUNS runs" > $BENCH_LOG
        for thread in $THREADS
        do
          PER_THREAD_STAT_FILE="$DIR/perf_stats-th$thread-ad$AD.txt"
        ...
        echo "Running passed periodic opt program: " >> $DEBUG_FILE
        BENCH_LOG="$DIR/$bench-th$thread-passed-periodic-runs-log.txt"
        echo "Periodic Passed - $RUNS runs" > $BENCH_LOG
        for thread in $THREADS
        do
          PER_THREAD_STAT_FILE="$DIR/perf_stats-th$thread-ad$AD.txt"
        ...
        #run eager passed LC
        echo "Building passed eager opt program: " >> $DEBUG_FILE
        BUILD_LOG=$BUILD_DEBUG_FILE ERROR_LOG=$BUILD_ERROR_FILE make -f Makefile.lc $bench-clean
        BUILD_LOG=$BUILD_DEBUG_FILE ERROR_LOG=$BUILD_ERROR_FILE CONFIG=3 ALLOWED_DEVIATION=$AD CLOCK_TYPE=0 PUSH_INTV=$PI CMMT_INTV=$CI INST_LEVEL=1 make -f Makefile.lc $bench
        echo "Running passed eager opt program: " >> $DEBUG_FILE
        BENCH_LOG="$DIR/$bench-th$thread-passed-eager-runs-log.txt"
        echo "Eager Passed - $RUNS runs" > $BENCH_LOG
        for thread in $THREADS
        do
          PER_THREAD_STAT_FILE="$DIR/perf_stats-th$thread-ad$AD.txt"
...
#perf_test water-nsquared water-spatial ocean-cp ocean-ncp barnes volrend fmm raytrace
