...
echo "Running benchmark for RAYTRACE-BALLS"

run_orig() {
ORIG_STAT_FILE="raytrace_balls_orig_stats.csv"
rm -f $ORIG_STAT_FILE
# Run original
...
do
  ...
  do
    ./RAYTRACE -p$threads -m72 inputs/balls4.env > out 2>/dev/null
    time_in_us=`cat out | grep "Total time without initialization" | sed "s/[^0-9]//g"`
