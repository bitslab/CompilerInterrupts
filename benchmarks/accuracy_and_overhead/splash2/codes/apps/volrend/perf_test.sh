...
echo "Running benchmark for VOLREND"

run_orig() {
ORIG_STAT_FILE="volrend_orig_stats.csv"
rm -f $ORIG_STAT_FILE
# Run original
...
DIVISOR=$NUMBER_OF_RUNS # Already in ms
...
do
  ...
  do
    ...
    time_in_us=`cat out | grep "wall clock execution time to compute normal:" | cut -d ':' -f 2 | cut -d' ' -f 3 | tr -d '[:space:]'`
