...
run_orig() {
  ...
  do
    ...
    time_in_us=`cat out | grep "volrend runtime: " | cut -d ':' -f 2 | cut -d ' ' -f 2 | tr -d '[:space:]'`
    cat out >> $LOG_FILE
...
run_opt() {
  ...
  if [ $naive_lc -gt $opt_lc ]; then
    err=`echo "scale = 3; (($naive_lc - $opt_lc) * 100 / $naive_lc)" | bc -l`
    echo "Err (in instrument count): $err% less" | tee -a $STAT_OUT_FILE
  else
    err=`echo "scale = 3; (($opt_lc - $naive_lc) * 100 / $naive_lc)" | bc -l`
