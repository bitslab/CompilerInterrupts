...
run() {
  suffix=$3
  echo "Running $1-$suffix with $2 threads for $NUMBER_OF_RUNS runs"
  DIVISOR=`expr $NUMBER_OF_RUNS \* 1000`
  rm -f sum
  echo -n "scale=2;(" > sum
  for j in `seq 1 $NUMBER_OF_RUNS`
  do
    ./$1-$suffix < input.$2 > out
    time_in_us=`cat out | grep "COMPUTETIME (after initialization) = " | cut -d '=' -f 2 | tr -d '[:space:]'`
    cat out >> $LOG_FILE
    echo $time_in_us | tr -d '\n' >> sum
    echo "$j. $time_in_us us"
    if [ $j -lt $NUMBER_OF_RUNS ]; then
      echo -n "+" >> sum
    fi
  done
  ...
  echo "Average duration: $time_in_ms"
}
