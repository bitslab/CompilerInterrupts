...
get_time() {
  ...
  if [ $# -eq 5 ]; then
    # Debug run
    prefix=$prefix"perf record --call-graph=dwarf -o $DIR/perf_${1}_th${threads}_${suffix}"
    #prefix=$prefix"perf stat -B -e cache-misses -o $DIR/perf_stat_${1}_th${threads}_${suffix}"
    #prefix=$prefix"perf stat -o $DIR/perf_stat_${1}_th${threads}_${suffix}"
  fi
  ...
  cat $OUT_FILE | grep "$1 runtime: " | tee -a $DEBUG_FILE
  cat $OUT_FILE | grep "CI took" | tee -a $DEBUG_FILE
}
...
orig_test() {
  ...
  do
    EXP_FILE="$DIR/splash2_orig-th$thread"
    echo "Orig" > $EXP_FILE
  done
  ...
  do
    ...
    *)
    ...
    do
      ...
      #orig_time=$(get_time $bench $thread 0 0 1) # 5th argument for debugging purpose
...
ci_perf_test() {
  ...
  case "$CI_SETTING" in
    1) ci_type="opt-local"
      AD=100
      echo "Evaluating OPT Local with ECC $AD" | tee -a $DEBUG_FILE $BUILD_DEBUG_FILE $BUILD_ERROR_FILE >/dev/null
    ;;
    2) ci_type="opt-tl"
      AD=100
  ...
  for thread in $THREADS
  do
    EXP_FILE="$DIR/splash2_${ci_type}-tuned-th$thread"
...
#CYCLE=5000
      ...
      BUILD_LOG=$BUILD_DEBUG_FILE ERROR_LOG=$BUILD_ERROR_FILE ALLOWED_DEVIATION=$AD CLOCK_TYPE=1 PUSH_INTV=$PI CMMT_INTV=$CI CYCLE_INTV=$CYCLE INST_LEVEL=$CI_SETTING EXTRA_FLAGS="-DPROFILE" make -f Makefile.lc $bench 
      #lc_periodic_time=$(get_time $bench $thread 1 0)
      #lc_periodic_time=$(get_time $bench $thread 1 0 1) # 5th argument for debugging purpose
      get_time $bench $thread 1 0
