	...
	uint32_t max_nonce, uint64_t *hashes_done)
{
  ...
  assert((n - first_nonce + 1 >= 0) && "Don't expect number of hashes to be less than 0");
#if 0
  if(total_hashes != total_hashes2) {
    printf("%d: Hashes don't match at the beginning. Total hashes: %lu, Total hashes 2: %lu\n", __LINE__, total_hashes, total_hashes2);
    exit(1);
  }
  printf("BEGIN %d: total_hashes: %lu, total hashes2: %lu, n: %d, first_nonce: %d\n", __LINE__, total_hashes, total_hashes2, n, first_nonce);
#endif
  total_hashes += n - first_nonce + 1;
  //printf("BEGIN 2 %d: total_hashes: %lu, total hashes2:%lu, n: %d, first_nonce: %d\n", __LINE__, total_hashes, total_hashes2, n, first_nonce);
...
#if 0
          total_hashes2 += *hashes_done;
          if(total_hashes2 != total_hashes) {
            printf("END 2 %d: total_hashes: %lu, total_hashes2: %lu, n: %d, first_nonce: %d\n", __LINE__, total_hashes, total_hashes2, n, first_nonce);
            printf("%d: Hashes don't match. Total hashes: %lu, Total hashes 2: %lu\n", __LINE__, total_hashes, total_hashes2);
            exit(1);
          }
#endif
					return 1;
...
#if 0
  total_hashes2 += *hashes_done;
  printf("END %d: total_hashes: %lu, total_hashes2: %lu, n: %d, first_nonce: %d\n", __LINE__, total_hashes, total_hashes2, n, first_nonce);
  if(total_hashes2 != total_hashes) {
    printf("%d: Hashes don't match. Total hashes: %lu, Total hashes 2: %lu\n", __LINE__, total_hashes, total_hashes2);
    exit(1);
  }
	...
	uint32_t max_nonce, uint64_t *hashes_done)
{
  ...
  printf("BEGIN %d: total_hashes: %lu, n: %d, first_nonce: %d\n", __LINE__, total_hashes, n, first_nonce);
#endif
  total_hashes += n - first_nonce + 1;
...
#if 0
  total_hashes2 += *hashes_done;
  printf("END %d: total_hashes: %lu, n: %d, first_nonce: %d\n", __LINE__, total_hashes, n, first_nonce);
  if(total_hashes2 != total_hashes) {
    printf("%d: Hashes don't match. Total hashes: %lu, Total hashes 2: %lu\n", __LINE__, total_hashes, total_hashes2);
    exit(1);
  }
