...
{
  printf("SETTING DROP POLICY FOR CI \n"); 
...
#if 1
static void affine_to_cpu(unsigned int core) {
#if 0
  printf("SETTING CPU AFFINITY FOR Thread %lu to Core %u\n", pthread_self(), core); 
  assert((core<64) && "Invalid core!");
  cpu_set_t set;
  CPU_ZERO(&set);
  CPU_SET(core, &set);
...
static void affine_to_cpu_mask(int id, unsigned long mask) {
  printf("SETTING CPU AFFINITY FOR CI \n"); 
  cpu_set_t set;
  CPU_ZERO(&set);
...
static void *workio_thread(void *userdata)
{
  init_stats_others();
  //init_stats();
  pthread_setname_np(pthread_self(), "workio");
  //affine_to_cpu(2);
  ...
  //printf("Signal %d received!\n", sig);
    ...
    if(fp != NULL) {
      ...
      if (diff.tv_usec || diff.tv_sec) {
			  double hashrate =
				  total_hashes / (diff.tv_sec + diff.tv_usec * 1e-6);
        sprintf(s, hashrate >= 1e6 ? "%.0f" : "%.2f",
          hashrate / 1e3);
        fprintf(fp, "%s\n", s);
        printf("Hashrate: %s, total hashes: %lu, hash rate: %.2lf, duration: %f sec\n", s, total_hashes, hashrate, diff.tv_sec + diff.tv_usec * 1e-6);

        /* resetting values */
        //total_hashes2=0;
        total_hashes = 0;
        g_tv_start.tv_sec = tv_end.tv_sec;
        g_tv_start.tv_usec = tv_end.tv_usec;
      }
      else {
        printf("No time has been spent between the two signals. This is abnormal. Exiting.\n");
        proper_exit(0);
      }
    }
    else {
      printf("File does not exist for thread %lu\n", pthread_self());
    }
    break;
	case SIGHUP:
...
{
  init_stats();
  pthread_setname_np(pthread_self(), "miner");
	...
	char s[16];
	int i;
  fp = fopen("cpuminer-hashrate", "w");
  if (fp == NULL) {
    printf("File for writing hashrate cannot be opened.");
    exit(1);
  }
  fprintf(fp, "Hash rate (kH/s)\n");
  printf("Created cpuminer-hashrate for miner thread %lu\n", pthread_self());
	...
	if (!opt_benchmark && opt_priority == 0) {
		...
		printf("Priority of miner thread: %d\n", getpriority(PRIO_PROCESS, 0));
		//drop_policy();
	} else {
		int prio = 0;
#ifndef WIN32
		...
		if (opt_priority == 0) {
      printf("REMOVE DROP POLICY IF THIS LINE OF CODE GETS EXECUTED FOR CPUMINER WITH IOKERNELD\n");
			drop_policy();
		}
	}
...
#if 1
#ifdef CI
  printf("NOT SETTING CPU AFFINITY FOR miner thread\n");
#else
  printf("NOT SETTING CPU AFFINITY FOR miner thread of orig thread\n");
	//affine_to_cpu(1);
#endif
#else
	if (num_cpus > 1) {
...
static void *longpoll_thread(void *userdata)
{
  init_stats_others();
  //init_stats();
  pthread_setname_np(pthread_self(), "longpoll");
  //affine_to_cpu(3);
	struct thr_info *mythr = (struct thr_info*) userdata;
	CURL *curl = NULL;
...
int main(int argc, char *argv[]) {
  init_stats_others();
  //init_stats();
  pthread_setname_np(pthread_self(), "cpuminer");
	//affine_to_cpu(5);
	struct thr_info *thr;
	long flags;
	int i, err;
    ...
    printf("CPU AFFINITY CONFIG IS DISABLED (%lx) !!", opt_affinity);
