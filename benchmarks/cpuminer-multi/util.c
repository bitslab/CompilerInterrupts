...
struct tq_ent {
	void			*data;
	struct list_head_cpuminer	q_node;
};

struct thread_q {
	struct list_head_cpuminer	q;
...
bool tq_push(struct thread_q *tq, void *data)
{
	...
	if (!tq->frozen) {
		list_add_tail_cpuminer(&ent->q_node, &tq->q);
	} else {
		free(ent);
		rc = false;
	}
