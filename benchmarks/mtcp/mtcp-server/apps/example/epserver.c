...
static __thread int accepted_cnt = 0;
static __thread int write_err = 0;
static __thread int partial_send_err = 0;
static __thread int epoll_timeout_errs = 0;
/*----------------------------------------------------------------------------*/
...
SendUntilAvailable(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
  ...
  if(sv->fsize != sv->total_sent) {
    partial_send_err++;
    TRACE_APP("Socket %d: mtcp_write left: %d, last sent: %d, fsz: %ld, sent: %d, total sent: %ld\n", sockid, len, ret, sv->fsize, sent, sv->total_sent);
  }
...
HandleReadEvent(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
	...
	if (http_header_str_val(sv->request, "Connection: ", 
		...
		if (strstr(keepalive_str, "Keep-Alive")) {
			...
			sv->keep_alive = FALSE; // this is used
		}
	}
	...
	TRACE_APP("Socket %d Sent response header: Header len: %d, try: %d, sent: %d\n", 
			sockid, HTTP_HEADER_LEN, len, sent);
	assert(sent == len);
...
InitializeServerThread(int core)
{
  ...
  printf("Calling mtcp_create_context with core id: %d\n", core);
...
RunServerThread(void *arg)
{
  init_stats();
    ...
    //RunMainLoop((void*)(mtcp_ctx->mtcp_thr_ctx));
        ...
        //fprintf(stderr, "CPU %d: Received MTCP_EPOLLERR, Closing connection\n", mtcp_ctx->cpu);
          ...
          fprintf(stderr, "CPU %d: Connection closed by remote host\n", mtcp_ctx->cpu);
					CloseConnection(ctx, events[i].data.sockid, 
							&ctx->svars[events[i].data.sockid]);
				} else if (ret < 0) {
					/* if not EAGAIN, it's an error */
					if (errno != EAGAIN) {
            fprintf(stderr, "CPU %d: %s\n", mtcp_ctx->cpu, strerror(errno));
						CloseConnection(ctx, events[i].data.sockid, 
								&ctx->svars[events[i].data.sockid]);
					}
				}
...
/*----------------------------------------------------------------------------*/
void
SignalHandlerResetCounter(int signum)
{
  fprintf(stdout,"Resetting counters for %d cores\n", core_limit);
  int i;
  for (i = 0; i < core_limit; i++) {
    //fprintf(stdout,"Before: Var of core %d: %lu\n", i, g_var[i].var);
#ifndef UNMOD
    g_var[i].var = 0;
#endif
    //fprintf(stdout,"After: Var of core %d: %lu\n", i, g_var[i].var);
  }
}
/*----------------------------------------------------------------------------*/
void
SignalHandler(int signum)
{
  ...
  exit(0);
	...
	for (i = 0; i < core_limit; i++) {
      ...
      //printf("Killing thread %d, g_var: %ld\n", i, g_var[i].var);
      if (!done[i]) {
        fprintf(stdout, "Thread %d - Accepted count: %d, Finished count: %d, write errors: %d, partial file send errors: %d, timeout errors: %d\n", i, accepted_cnt, finished, write_err, partial_send_err, epoll_timeout_errs);
#ifndef UNMOD
        fprintf(stdout, "Counter:%d,%lu\n",i,g_var[i].var);
#endif
      }
...
main(int argc, char **argv)
{
...
#ifndef INFO
  fprintf(stderr,"Info not defined\n");
  exit(1);
#else
  fprintf(stderr,"Info defined\n");
  TRACE_INFO("TRACE_INFO works!\n");
  TRACE_ERROR("TRACE_ERROR works!\n");
  TRACE_APP("TRACE_APP works!\n");
  TRACE_CONFIG("TRACE_CONFIG works!\n");
#endif
  ...
  init_stats();
	...
	mtcp_register_signal(SIGUSR2, SignalHandlerResetCounter); // SIGUSR1 is used in mtcp
