...
HandleReadEvent(thread_context_t ctx, int sockid, struct wget_vars *wv)
{
    ...
    int rem_file_len = rd;
		if (rd <= 0) {
      //fprintf(stderr, "mtcp_read returned 0\n");
			break;
    }
		ctx->stat.reads += rd;
	...
	if (rd > 0) {
		...
		if (wv->header_len && (wv->recv >= wv->header_len + wv->file_len)) {
      //printf("Download Completed. Closing connection.\n");
			DownloadComplete(ctx, sockid, wv);
		} else {
...
#ifndef CI
	DestroyContext(ctx); // to retain stats till the end of the program, this call is disabled
#endif
