...
run_client_for_conc() {
  ...
  for i in `seq 1 $RUNS`; do
    ...
    cmd="time ./epwget $server_ip/NOTES $REQUESTS -N $THREADS -c $CONCURRENCY -f ${client}_epwget.conf 2>&1 | tee out"
...
if [ "$client" == "lines" ]; then
  ...
  server_path="~/logicalclock/ci-llvm-v9/test-suite/mtcp/mtcp-server/"
elif [ "$client" == "quads2" ]; then
  server="quads1"
  server_ip="192.168.1.1"
  server_path="~/logicalclock/ci-llvm-v9/test-suite/mtcp/mtcp-mellanox-server/"
else
