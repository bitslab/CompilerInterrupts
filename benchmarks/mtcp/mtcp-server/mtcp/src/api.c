	...
	if (!accepted) {
		if (listener->socket->opts & MTCP_NONBLOCK) {
			...
			while ((accepted = StreamDequeue(listener->acceptq)) == NULL) {
				pthread_cond_wait(&listener->accept_cond, &listener->accept_lock); // does not get used
...
#elif defined(HYBRID) || defined(NO_BATCHING)
	struct timeval cur_ts = {0};
	gettimeofday(&cur_ts, NULL);
	uint32_t ts = TIMEVAL_TO_TS(&cur_ts);
	AddtoControlList(mtcp, cur_stream, ts);
#else
...
CloseStreamSocket(mctx_t mctx, int sockid)
{
...
#if defined(HYBRID) || defined(NO_LOCKS)
  ...
  gettimeofday(&cur_ts, NULL);
  uint32_t ts = TIMEVAL_TO_TS(&cur_ts);
  HandleCloseCalls(cur_stream,mtcp,ts,&handled, &delayed, &control, &send, &ack);
  TRACE_ROUND("Handling close connection.\n");
  HandleCloseCallsInt(mtcp, ts);
  ret = 0;
#else
...
mtcp_abort(mctx_t mctx, int sockid)
{
  ...
  gettimeofday(&cur_ts, NULL);
  uint32_t ts = TIMEVAL_TO_TS(&cur_ts);
  HandleResetCalls(cur_stream,mtcp,ts);
  TRACE_ROUND("Handling reset connection.\n");
  HandleResetCallsInt(mtcp, ts);
  ret = 0;
#else
...
CopyToUser(mtcp_manager_t mtcp, tcp_stream *cur_stream, char *buf, int len)
{
	...
	if (cur_stream->need_wnd_adv) {
        ...
        gettimeofday(&cur_ts, NULL);
        uint32_t ts = TIMEVAL_TO_TS(&cur_ts);
        cur_stream->sndvar->on_ackq = FALSE;
        EnqueueACK(mtcp, cur_stream, ts, ACK_OPT_AGGREGATE);
#else
