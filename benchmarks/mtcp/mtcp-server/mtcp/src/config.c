...
static const char *route_file_name = 		"route.conf";
static const char *arp_file_name = 			"arp.conf";
...
#define ROUTES "ROUTES"
  ...
  if(gethostname(hostbuffer, sizeof(hostbuffer)) != -1)
    ...
    printf("Hostname is unknown. Exiting\n");
    exit(-1);
  }

  char route_file[512];
  sprintf(route_file, "%s/%s_%s", config_path, hostbuffer, route_file_name);
...
#define ARP_ENTRY "ARP_ENTRY"
  ...
  char arp_file[512];
  sprintf(arp_file, "%s/%s_%s", config_path, hostbuffer, arp_file_name);
