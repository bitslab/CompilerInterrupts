...
HandleResetCallsInt(mtcp_manager_t mtcp, uint32_t cur_ts) {
	int cnt = 0;
	int max_cnt = mtcp->resetq_int->count;
  tcp_stream *stream;
	while (cnt++ < max_cnt) {
...
HandleCloseCallsInt(mtcp_manager_t mtcp, uint32_t cur_ts) {
	int cnt = 0;
	int max_cnt = mtcp->closeq_int->count;
  tcp_stream *stream;
	while (cnt++ < max_cnt) {
...
#if !defined(HYBRID) && !defined(NO_BATCHING)
	/* connect handling */
...
#if !defined(NO_BATCHING)
    HandleCloseCalls(stream, mtcp, cur_ts, &handled, &delayed, &control, &send, &ack);
#endif
	}
	TRACE_ROUND("Handling close connections. cnt: %d\n", cnt);
#if !defined(NO_BATCHING)
  HandleCloseCallsInt(mtcp, cur_ts);
#endif

	/* reset handling */
	while ((stream = StreamDequeue(mtcp->resetq))) {
#if !defined(NO_BATCHING)
    HandleResetCalls(stream, mtcp, cur_ts);
#endif
	}
	TRACE_ROUND("Handling reset connections. cnt: %d\n", cnt);
#if !defined(NO_BATCHING)
    HandleResetCallsInt(mtcp, cur_ts);
#endif
...
mtcp_setconf(const struct mtcp_conf *conf)
{
	...
	TRACE_CONFIG("Configuration updated by mtcp_setconf(). Concurrency: %d\n", CONFIG.max_concurrency);
	//PrintConfiguration();
...
mtcp_init(const char *config_file)
{
  ...
  #if defined(HYBRID) || defined(NO_BATCHING)
    TRACE_INFO("HYBRID or NO_BATCHING is used!!!!!\n");
  #endif
  #if !defined(HYBRID) && !defined(NO_BATCHING)
    TRACE_INFO("HYBRID and NO_BATCHING is not used!!!!!\n");
  #endif
  #if defined(HYBRID)
    TRACE_INFO("HYBRID is used!!!!!\n");
  #elif defined(NO_BATCHING)
    TRACE_INFO("NO_BATCHING is used!!!!!\n");
  #elif defined(NO_LOCKS)
    TRACE_INFO("NO_LOCKS is used!!!!!\n");
  #else
    TRACE_INFO("original version is used!!!!!\n");
  #endif
