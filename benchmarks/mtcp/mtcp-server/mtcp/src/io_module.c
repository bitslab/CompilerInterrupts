...
probe_all_rte_devices(char **argv, int *argc, char *dev_name_list)
{
  ...
  *argc+=2;
  argv[*argc-2]=strdup("-w");
  argv[*argc-1]=strdup("0000:af:00.0");
...
SetNetEnv(char *dev_name_list, char *port_stat_list)
{
    ...
    printf("After rte_eal_init: Found %d port(s)\n", i);
    ...
    RTE_ETH_FOREACH_DEV(pi) {
      printf("Found a valid port!!!\n");
			rte_eth_macaddr_get(pi, &ports_eth_addr[ret]);
      ret++;
    }
    printf("Max ports: %d, Actual ports: %d\n", RTE_MAX_ETHPORTS, ret);
    if(ret>RTE_MAX_ETHPORTS) {
      printf("Seems like an error. Actual ports exceeded max ports!\n");
    }
#endif
        ...
        printf("Configured device %s (index:%d) is found in dpdk enabled devices!\n", 
