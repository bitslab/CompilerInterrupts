...
MPCreate(char *name, int chunk_size, size_t total_size)
{
	...
	if (mp == NULL) {
		TRACE_ERROR("Can't allocate memory for mempool for socket id %d (name %s)! Total size: %ld, chunk size: %d, size reqd: %ld\n", rte_socket_id(), name, total_size, chunk_size, sz);
		exit(EXIT_FAILURE);
	}
