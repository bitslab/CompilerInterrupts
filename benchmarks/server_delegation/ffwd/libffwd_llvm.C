...
inline void __attribute__((always_inline)) server_inner_poll(){
	...
	this_server->tot_ops += this_server->resp_ready;
...
#ifdef PTHREAD
	//TODO: Keep count of clients created so far and fail if exceed TOTAL_NUM_OF_THREADS
	clients_count++;
	assert(clients_count <= TOTAL_NUM_OF_THREADS - num_of_server_launched);
...
void ffwd_init() {
	...
	//Check if num_of_req_lines_to_check > TOTAL_NUM_OF_THREADS
	...
	for (i = 0; i < TOTAL_NUM_OF_THREADS+1; i++){
		...
		if (i==TOTAL_NUM_OF_THREADS){
			...
			thread_attrs[thread_id] = (struct thread_attr *) numa_alloc_onnode(sizeof(struct thread_attr), 0);
		}
		else{
			thread_id = all_threads[i];
			thread_attrs[thread_id] = (struct thread_attr *) numa_alloc_onnode(sizeof(struct thread_attr), (i/MAX_THREADS_PER_SOCK));
		}
...
fiber_t* launch_server_and_pin(int server_core){
	...
	server_response_group_set[num_of_server_launched] = (struct server_set*)numa_alloc_onnode(sizeof(struct server_set), server_numa_node);
	for (i = 0; i < eval(MAX_SOCK * RESP_GROUP_PER_SOCK); i++){ 
		(server_response_group_set[num_of_server_launched]->server_response_groups[i]) = (struct server_response_group*)numa_alloc_onnode(sizeof(struct server_response_group), server_numa_node);
	...
	forloop(`chip', `0', MAX_SOCK-1, 
	``chip'chip[num_of_server_launched] = (struct request*)numa_alloc_onnode(REQ_MEMORY_SIZE_ALIGNED, chip);
	...
	server_arg[num_of_server_launched] = (struct server_args*) numa_alloc_onnode(sizeof(struct server_args), server_numa_node);
	...
	server_arg[num_of_server_launched]->server_local_resp_flags = numa_alloc_onnode(MAX_SOCK*sizeof(uint64_t*), server_numa_node);
	server_arg[num_of_server_launched]->server_local_responses = numa_alloc_onnode(MAX_SOCK*sizeof(struct server_response *), server_numa_node);

	forloop(`chip', `0', MAX_SOCK-1, 
	`server_arg[num_of_server_launched]->server_local_resp_flags[chip] = numa_alloc_onnode((eval((REQS_LINES_PER_THREAD * MAX_THREADS_PER_SOCK)/64 + eval((REQS_LINES_PER_THREAD * MAX_THREADS_PER_SOCK)%64>0)))*sizeof(uint64_t), server_numa_node);
	server_arg[num_of_server_launched]->server_local_responses[chip] = numa_alloc_onnode(CLIENT_PER_RESPONSE*RESP_GROUP_PER_SOCK*sizeof(struct server_response), server_numa_node);
	...
	server_arg[num_of_server_launched]->tot_ops = 0;
