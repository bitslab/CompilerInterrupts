...
typedef struct dist_fifo
{
    ...
    int counter;
    char _cache_padding2[CACHE_SIZE - 4 - sizeof(dist_fifo_node_t*)];
} __attribute__((__packed__)) dist_fifo_t;

static inline int dist_fifo_init(dist_fifo_t** f)
{
    // assert(fifo);
    ...
    dist_fifo_t* fifo = (dist_fifo_t*) malloc (sizeof(dist_fifo_t));
...
static inline int compare_and_swap3(volatile pointer_pair_t* location, const pointer_pair_t* original_value, const pointer_pair_t* new_value){
        if ( location->high ==  original_value->high) {
            location->high = new_value->high;
        }
        else{
...
// static inline dist_fifo_node_t* dist_fifo_loop(dist_fifo_t* fifo){
//     int i;
//     assert(fifo);
...
//         compare_and_swap3(&fifo->head.blob, &old_head.blob, &new_head.blob);

//         // if(!compare_and_swap3(&fifo->head.blob, &old_head.blob, &new_head.blob)) {
//         //     return DIST_FIFO_RETRY;
//         // }
//         prev_head->data = data;
//         fifo->counter--;
