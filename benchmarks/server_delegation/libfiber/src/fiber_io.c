...
// "epoll" relates stuff
int __epoll_create_internal(int size)
{
    if(!fibershim_epoll_create)
        fibershim_epoll_create = (epoll_createFnType)dlsym(RTLD_NEXT,"epoll_create");

    return fibershim_epoll_create(size);
}


int epoll_create(int size)
{
    return fiber_epfd_create(size);
}


int __epoll_ctl_internal(int epfd, int op, int fd, struct epoll_event *event)
{
    if(!fibershim_epoll_ctl)
        fibershim_epoll_ctl = (epoll_ctlFnType)dlsym(RTLD_NEXT,"epoll_ctl");

    return fibershim_epoll_ctl(epfd, op, fd, event);
}


int epoll_ctl(int epfd, int op, int fd, struct epoll_event *event)
{
    return fiber_epfd_ctl( epfd, op, fd, event);
}


int __epoll_wait_internal(int epfd, struct epoll_event *events, int maxevents, int timeout)
{
    if(!fibershim_epoll_wait)
        fibershim_epoll_wait = (epoll_ctlFnType)dlsym(RTLD_NEXT,"epoll_wait");

    return fibershim_epoll_wait(epfd, events, maxevents, timeout);
}


int epoll_wait(int epfd, struct epoll_event *events, int maxevents, int timeout)
{
    return fiber_epfd_wait( epfd, events, maxevents, timeout);
}