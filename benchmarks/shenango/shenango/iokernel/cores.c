...
static inline void core_reserve(unsigned int core, struct thread *th)
{
  ...
  if(core_history[core].next && th != core_history[core].next) {
    printf("%d: Core: %d, Core history next: %p, Thread chosen: %p\n", __LINE__, core, core_history[core].next, th);
  }
...
bool cores_park_kthread(struct thread *th, bool force)
{
  ...
  if(bitmap_test(avail_cores, core)) {
    printf("%d: available cores bitmap: %lu, core: %d\n", __LINE__, *avail_cores, core);
  }
	BUG_ON(bitmap_test(avail_cores, core));
  if(bitmap_test(p->available_threads, kthread)) {
    printf("%d: available threads bitmap: %lu, kthread index: %d\n", __LINE__, *p->available_threads, core);
  }
...
struct thread *cores_add_core(struct proc *p)
{
  ...
  if(!bitmap_test(p->available_threads, th - p->threads)) {
    printf("%d: Core to be assigned: %d, Index of thread to be assigned: %ld, Bitmap: %lu\n", __LINE__, core, th - p->threads, *p->available_threads);
  }
  ...
  preempt_sig++;
    ...
    printf("USR1 signal to pid %d tid %d failed. Error: %s\n", th_current->p->pid, th_current->tid, strerror(errno));
		WARN();
	}
...
void cores_adjust_assignments(void)
{
	...
	for (i = 0; i < dp.nr_clients; i++) {
		...
		if (p->active_thread_count < p->sched_cfg.guaranteed_cores) {
#ifdef DBG
      if(active_thrds < p->active_thread_count)
        active_thrds = p->active_thread_count;
      gcores = p->sched_cfg.guaranteed_cores;
      core_add2++;
#endif
			cores_add_core(p);
    }
...
int cores_init(void)
{
  ...
  printf("dp core: %d, linux core: %d, control core: %d\n", 
      core_assign.dp_core, core_assign.linux_core, core_assign.ctrl_core);
  ...
  printf("Number of cores initialized: %d\nCores are: ", k);
  for(l=0;l<k;l++) {
    printf("%d\t", (int)b[l]);
  }
  printf("\n");
